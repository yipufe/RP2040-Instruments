...
void loop() {
  while( Serial.available() ) {
    ...
    if(nextChar=='h') {
      int currentHeading = readHeading(3);
      //Idle mode off
      drawFullHeadingIndicator(currentHeading%360);
    }
    while(Serial.available()) {
      Serial.read();
    }
...
int readHeading(int place) {
  while(Serial.available()) {
    ...
    if(place==1) {
      ...
      return (int)(c-48)*10+readHeading(place-1);
    } else {
      return (int)(c-48)*100+readHeading(place-1);
    }
  }
  return 0;
}
...
void drawFullHeadingIndicator(int deg) {
    ...
    for(int x=0;x<240;x++) {
      for(int y=239;y>=0;y--) {
        int rotX=((int)(rotationPartCos(degForCos,x) - rotationPartSin(deg,y))>>8)+120;//((rotationPartCos(degForCos,x) - rotationPartSin(deg,y))>>8)+120;
        int rotY=((int)(rotationPartSin(deg,x) + rotationPartCos(degForCos,y))>>8)+120;//((rotationPartSin(deg,x) + rotationPartCos(degForCos,y))>>8)+120;
        //if(rotX>240||rotX<0||rotY>240||rotY<0) {
          //Nothing to see here
        //} else {
            int iRotate=rotX*4+rotY*960;

            i+=2;
            unsigned int pixel1 = pxl(background[iRotate],background[iRotate+1],background[iRotate+2]);
            //unsigned char alpha1 = background[iRotate+3];
            ...
            unsigned int mixedValue2 = mixAlpha(mixedValue1, pixel2, alpha2);
...
unsigned int mixAlpha(unsigned int pxl1, unsigned int pxl2, unsigned char alpha) {
  ...
  unsigned char pxl_red_combo = (unsigned char)((alphaScale(pxl1_red,255-alpha) + alphaScale(pxl2_red,alpha))>>12);
  //if(pxl_red_combo > 0x1f) {
    //pxl_red_combo = 0x1f;
  //}
  unsigned char pxl_green_combo = (unsigned char)((alphaScale(pxl1_green,255-alpha) + alphaScale(pxl2_green,alpha))>>12);
  //if(pxl_green_combo > 0x3f) {
    //pxl_green_combo = 0x3f;
  //}
  unsigned char pxl_blue_combo = (unsigned char)((alphaScale(pxl1_blue,255-alpha) + alphaScale(pxl2_blue,alpha))>>12);
