...
void drawFullTurnCoordinatorIndicator(int plane, int ball);
...
void drawLine(int x, int y, int len, int halfWidth, unsigned int color, unsigned char alpha);
...
void setup() {
  ...
  drawFullTurnCoordinatorIndicator(0, 0);
...
void loop() {
  while( Serial.available() ) {
    if(Serial.read()=='r') {
      currentRate = readNumber(3);
      //drawFullAirSpeedIndicator(currentValue);
      while(Serial.available()) {
        if( Serial.read() == 'b' ) {
          currentBall = readNumber(3);
          drawFullTurnCoordinatorIndicator(currentRate, currentBall);
...
void drawFullTurnCoordinatorIndicator(int rate, int ball) {
    ball *= (55.0/128);
    int deg = rate;
    deg=deg+360;
    deg%=360;
    ...
    unsigned int pxlBackground;
    unsigned char alphaBackground;
    unsigned int carryBackground=1;
    int iFixedBackground=0;
    
    unsigned int pxlBackplate;
    unsigned char alphaBackplate;
    unsigned int carryBackplate;
    int iFixedBackplate=0;

    unsigned int backplatePixel;
    unsigned char backplateAlpha;
    ...
    for(int x=0;x<240;x++) {
      for(int y=239;y>=0;y--) {
        ...
        if(--carryBackground<=0) {
          pxlBackground = backplateImg[iFixedBackground++];
          //pxlBackground = backgroundImg[iFixedBackground++];
          iFixedBackground++;
          carryBackground = backplateImg[iFixedBackground++];
          //carryBackground = backgroundImg[iFixedBackground++];
        }

        int iter=x*4+y*960;
        unsigned int ballPixel;
        unsigned char ballAlpha;
        int xAdjusted = (x-(abs(ball)>>4));
        int yAdjusted = (y+ball);
        if(xAdjusted<57 || xAdjusted>87 || yAdjusted<105 || yAdjusted>135) {
          ballAlpha = 0x00;
        } else {
          int iterBall=xAdjusted*4+yAdjusted*960;
          ballPixel = pxl(ballImg[iterBall],ballImg[iterBall+1],ballImg[iterBall+2]);
          ballAlpha = ballImg[iterBall+3];
        }
  
        unsigned int mixedValue1 = mixAlpha(pxlBackground, ballPixel, ballAlpha);
/*
        backplatePixel = pxl(backplateImg[iter],backplateImg[iter+1],backplateImg[iter+2]);
        backplateAlpha = backplateImg[iter+3];


        unsigned int mixedValue2 = mixAlpha(mixedValue1, backplatePixel, backplateAlpha);
*/
        int rotX=0;
        ...
        if(rotY<103 || rotY>155) {
          
        } else {
          rotX=((int)(rotationPartSin(deg,x) + rotationPartCos(degForCos,y))>>8)+120;//((rotationPartSin(deg,x) + rotationPartCos(degForCos,y))>>8)+120;
        }

        unsigned int planePixel;
        unsigned char planeAlpha;
        if(rotY<103 || rotY>155 || rotX<30 || rotX>210) {
          planeAlpha = 0x00;         
        } else {
          int iRotate=rotY*4+rotX*960;
          planePixel = pxl(planeImg[iRotate],planeImg[iRotate+1],planeImg[iRotate+2]);
          planeAlpha = planeImg[iRotate+3];
        }

        unsigned int mixedValue3 = mixAlpha(mixedValue1, planePixel, planeAlpha);
 
        i+=2;       
        currentImage[i+1] = mixedValue3&0xff;
        currentImage[i] = mixedValue3>>8;

      }
    }
    drawLine(106,59,30,1,0x0000,0xff);
    drawLine(136,59,30,1,0x0000,0xff);
